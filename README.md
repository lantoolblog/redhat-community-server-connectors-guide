...
VSCode는 많이 복잡해진 상태라... 건들이기가 싫어서 Antigravity IDE에서 확인해보기로 했다.
...
* 샘플 프로젝트에 워크스페이스 설정에 약간의 편의를 위한 maven 확장용 즐겨찾는 명령과 rsp 관련 설정을 **.vscode/settings.json**에 미리 넣어놨음.
  ...
  * rsp-ui.rsp.java.home은... 프로젝트의 Java 버전과 일치하게 하면 됌.
...
나는 아래 처럼 G:\드라이브에다 redhat-community-server-connector라는 폴더를 만들고, apache-tomcat-10.1.55-windows-x64.zip 압축파일을 공식 tomcat 홈페이지에서 받아다 풀었음.
...
다운로드 할꺼냐고 묻는데, 위에서 미리 다운로드 받은것을 쓸 것 이므로 No 선택
...
`{프로젝트 루트}\target\spring-mvc-sample` 이 경로를 선택하면됌
...
Tomcat 10.1.55 부분에서 오른쪽 마우스 메뉴 열어서 Start Server 눌러주자!!
